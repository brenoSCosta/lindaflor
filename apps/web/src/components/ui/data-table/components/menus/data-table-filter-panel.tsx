...
import {
  ...
  isValuelessOperator,
} from "@/components/ui/data-table/fns/advanced-filter";
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    <Select
                      value={rule.operator}
                      onValueChange={(v) => {
                        const nextMode = operators.find((op) => op === v);
                        if (nextMode) changeOperator(rule, nextMode);
                      }}
                    >
...
function RuleValueInput<TData extends RowData>({
  ...
  if (variant === "checkbox") {
    return (
      <Select
        value={rule.value === undefined ? "" : stringifyRuleValue(rule.value)}
    ...
    return (
      <div className="flex flex-1 items-center gap-1">
        ...
        <Input
          type={type}
          className="h-8"
          value={stringifyRuleValue(rule.value2)}
