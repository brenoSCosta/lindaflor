 ...
 */
function getRank(meta: unknown): number | undefined {
  if (typeof meta !== "object" || meta == null) return undefined;
  if (!("rank" in meta)) return undefined;
  const rank = Reflect.get(meta, "rank");
  return typeof rank === "number" ? rank : undefined;
}

function best<TData extends RowData>(row: Row<TData>): number {
  let max = -Infinity;
  for (const meta of Object.values(row.columnFiltersMeta)) {
    const rank = getRank(meta);
...
export function createRankedSortedRowModel<TData extends RowData>(
  ...
  return (table) => {
    ...
    return () => {
      ...
      cached = { ...model, rows: model.rows.toSorted(rankGlobalFuzzy) };
      cachedFor = model;
      return cached;
    };
  };
